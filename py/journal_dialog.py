...
class AddWorkoutDialog(Gtk.Dialog):
    def __init__(self, journal_tab):
        super().__init__(title="Add Workout", transient_for=journal_tab.get_toplevel(), modal=True)
        ...
        walking_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=10, margin=10)
        notebook.append_page(walking_box, Gtk.Label(label="Walking"))

        self.distance_entry = Gtk.Entry(placeholder_text="Distance km")
        self.distance_entry.connect("changed", self._on_numeric_entry_changed)
        walking_box.pack_start(self.distance_entry, False, False, 0)

        self.weight_entry = Gtk.Entry(placeholder_text="Weight kg")
        ...
        walking_box.pack_start(self.weight_entry, False, False, 0)
        ...
        walking_box.pack_start(self.add_to_selected_check, False, False, 0)
    ...
    def on_add_clicked(self, widget):
        distance_text = self.distance_entry.get_text().strip().replace(',', '.')
        weight_text = self.weight_entry.get_text().strip().replace(',', '.')
        add_to_selected = self.add_to_selected_check.get_active()

        if not distance_text:
            self._show_error("Enter distance in km")
            return False

        if not weight_text:
            self._show_error("Enter your weight in kg")
            return False
        ...
        try:
            ...
            self._show_error("Invalid values - must be numbers (e.g. 5 or 5.5)")
            return False

        kcal = 0.5 * weight * distance
        ...
        entry = {
            'timestamp': timestamp,
            'date': date,
            'ate': f"Walk: {distance:.2f}km",
            'kcal': -kcal,
            'weight': weight
        }

        self.journal_tab.journal_data.append(entry)
        self.journal_tab._save_journal()
...
class AddEntryDialog(Gtk.Dialog):
    def __init__(self, journal_tab):
        ...
        self.ingredients_view.set_size_request(-1, 150)
        ...
        self.recipes_view.set_size_request(-1, 150)
        renderer = Gtk.CellRendererText()
        ...
        weight_box.pack_start(Gtk.Box(), True, True, 0)
