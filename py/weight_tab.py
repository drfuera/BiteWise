...
class WeightGraph(Gtk.DrawingArea):
    ...
    def on_draw(self, widget, cr):
        ...
        # Calculate reasonable weight range
        min_weight = max(50, min(weights) * 0.95)  # Minimum of 50kg or 5% below lowest weight
        max_weight = min(150, max(weights) * 1.05)  # Maximum of 150kg or 5% above highest weight
        ...
        # Draw points and hover indicator
        point_radius = 5
        if num_dates <= 100:
            cr.set_source_rgba(*weight_color)
    ...
    def on_motion_notify(self, widget, event):
        ...
        for i, (x, y) in enumerate(self.graph_points):
            ...
            if dist < min_dist and dist < 400:  # 20^2 = 400
                min_dist = dist
                closest_point = i
...
class WeightStatsTab(Gtk.Box):
    ...
    def update_plot(self):
        """Update the weight plot with fresh data"""
        self.journal_data = self._load_journal_data()
        self.daily_weights = self._process_weight_data()
        self.create_weight_plot()
